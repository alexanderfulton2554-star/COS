    ...
    <main className="min-h-screen bg-[#f6f8f6] text-slate-900">
      <section className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-10">
        <div className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
          <div className="relative min-h-[520px] overflow-hidden bg-slate-900 px-4 py-6 text-white sm:min-h-[560px] sm:px-10 sm:py-8 lg:px-14">
            ...
            <div className="relative z-10 flex h-full min-h-[468px] flex-col sm:min-h-[500px]">
              ...
              <div className="mt-auto max-w-2xl pb-4 pt-20">
                ...
                <h1 className="mt-5 max-w-2xl text-3xl font-semibold leading-tight tracking-tight sm:text-6xl">A welcoming place to live, work, and feel cared for.</h1>
                ...
                <div className="mt-8 grid gap-3 sm:flex sm:flex-wrap"><Link href="/register" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-900 hover:bg-sky-50">Explore opportunities <ArrowRight className="h-4 w-4" /></Link><Link href="#services" className="rounded-full border border-white/30 px-5 py-3 text-center text-sm font-semibold text-white hover:bg-white/10">Discover our care</Link></div>
              </div>
            </div>
          </div>
        ...
        <section id="services" className="py-10 sm:py-14"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-700">What we provide</p><h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">Support for every stage of care.</h2></div><a href="#contact" className="text-sm font-semibold text-sky-700 hover:text-sky-600">Talk to our team <ArrowRight className="ml-1 inline h-4 w-4" /></a></div><div className="mt-7 grid gap-4 md:grid-cols-3">{services.map(({ title, description, icon: Icon, tone }) => <div key={title} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${tone}`}><Icon className="h-6 w-6" /></div><h3 className="mt-5 text-xl font-semibold text-slate-900">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{description}</p></div>)}</div></section>
