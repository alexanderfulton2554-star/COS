  ...
  async function submitPayment(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/payments/crypto", {
        ...
        body: JSON.stringify({ applicationRef, network, txHash }),
      });
  ...
  return (
    <form onSubmit={submitPayment} className="mt-3 space-y-3">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500">Send exactly 50 USDT on {network} to</p>
      ...
      <button type="submit" disabled={loading} className="rounded-xl bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-500 disabled:opacity-60">{loading ? "Submitting..." : "Submit transaction for review"}</button>
